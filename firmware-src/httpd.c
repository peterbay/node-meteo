...
static void httpd_send_file_part ( void *arg )
{
  NODE_DBG_HTTPD ( "FUNC: httpd_send_file_part\n" );
  ...
  espconn_regist_sentcb ( pesp_conn, httpd_send_file_part );

  socket_userdata *socket = (socket_userdata *) pesp_conn->reverse;

  if ( socket == NULL || socket->uri == NULL || socket->file_size == 0 || socket->file_offset == 0 )
  {
    espconn_disconnect ( arg );
    return;
  }
  ...
  err2 = vfs_lseek ( f, socket->file_offset, VFS_SEEK_SET );

  if ( err2 != VFS_RES_ERR )
  {
    file_read = socket->file_size - socket->file_offset;
    ...
    response_file = (char *) c_malloc ( file_read );
  
    if ( response_file != NULL)
    {
      vfs_read  ( f, response_file, file_read );
      vfs_close ( f );

      socket->file_offset += file_read;

      if ( socket->file_size <= socket->file_offset )
      {
        espconn_regist_sentcb ( pesp_conn, httpd_socket_sent_close_callback );
      }

      espconn_send ( pesp_conn, (unsigned char *) response_file, file_read );
...
static void httpd_send_file ( void *arg, char *filename, bool enable_etag )
{
  ...
  if ( err2 != VFS_RES_ERR )
  {
    ...
    if ( file_size > 2800 )
    {
      ...
      espconn_regist_sentcb ( pesp_conn, httpd_send_file_part );
      socket->file_size = file_size;
      socket->file_offset = 2800;
      int filename_len = c_strlen ( filename );
      socket->uri = c_zalloc ( filename_len + 1 );
      if ( socket->uri == NULL )
      {
        espconn_send ( pesp_conn, (unsigned char *) http_header_501, (uint16) c_strlen ( http_header_501 ) );
        return;
      }
      c_memcpy ( socket->uri, filename, filename_len );
      file_read = 2800;
    }
    
    response_file = (char *) c_malloc ( file_read + 120 );
  
    if ( response_file != NULL)
    {
      response_header_length = c_sprintf ( response_file, "HTTP/1.1 200 OK\r\n%sContent-length: %d\r\nContent-Type: %s\r\n\r\n", etag, file_size, httpd_mime[ mime ] );
      vfs_read  ( f, &( response_file[response_header_length] ), file_read );
      vfs_close ( f );

      espconn_send ( pesp_conn, (unsigned char *) response_file, response_header_length + file_read );
      c_free ( response_file );
